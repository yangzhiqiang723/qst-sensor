		...
		struct device_attribute *attr, char *buf)
{
	...
	qmaX981_read_raw_xyz(qmaX981, sensordata);
			       ...
			       const char *buf, size_t count)
{
	...
	qmaX981->enable_flag = en;
	if (en) 
	{
		ret = qmaX981_power_set(qmaX981, true);
		if (ret) {
			MSE_ERR("Fail to power on the device!\n");
			goto mutex_exit;
		}
		ret = qma6981_initialize(qmaX981->i2c);
		if (ret < 0)
			goto mutex_exit;
		schedule_delayed_work(&qmaX981->dwork, (unsigned long)nsecs_to_jiffies64(qmaX981->delay));
	}
	else
	{
		cancel_delayed_work_sync(&qmaX981->dwork);
		#ifndef QMAX981_STEPCOUNTER
		ret = qmaX981_power_set(qmaX981, false); //keep power on for sc	
		if (ret)
		{
			MSE_ERR("Fail to power off the device!\n");
			goto mutex_exit;
...
//	dat_buf[0], dat_buf[1], dat_buf[2], dat_buf[3],dat_buf[4], dat_buf[5]);
	...
	data[QMA6981_AXIS_X] = (acc_layout[QMA6981_AXIS_X]*1000)/qma->resolution;
	data[QMA6981_AXIS_Y] = (acc_layout[QMA6981_AXIS_Y]*1000)/qma->resolution;
	data[QMA6981_AXIS_Z] = (acc_layout[QMA6981_AXIS_Z]*1000)/qma->resolution;
